...
export default function UserBox(props: Props) {
  ...
  if (props.rankStyle) {
    rankClass +=
      props.rankStyle === "word" ? " is-word" : ` is-${props.rankStyle}`;
  }
  return (
    <Link
      className={`hp-row${hasRank ? "" : " is-norank"}`}
      href={`/picks/${props.user.uuid}`}
    >
      {hasRank && <div className={rankClass}>{props.rank}</div>}
      ...
      <div className="hp-row-stats">{props.children}</div>
    </Link>
  );
}
