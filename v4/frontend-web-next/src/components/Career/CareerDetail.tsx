import { Player, Season, TeamStat } from "@/lib/types";
import { place, signed } from "@/lib/format";
  ...
  return (
    <div className="career-team">
      <div className="career-team-logo">
        <Logo src={s.team.logo} alt={s.team.name} size={26} />
      </div>
      <div className="career-team-name">{s.team.name}</div>
      <div className="career-team-count">{`${s.count}×`}</div>
      <div
        className={`career-team-record${props.tone ? ` is-${props.tone}` : ""}`}
      >
        ...
        <div className="career-panel">
          ...
          <div className="career-seasons">
            ...
            {player.seasons.map((s: Season) => (
              <div
                className={`career-season${s.champion ? " is-champion" : ""}`}
                key={s.year}
              >
                <span className="career-season-year">
                  {s.year}
                  {s.champion && <span className="career-trophy">🏆</span>}
                </span>
                <span className="hp-num">{`${s.wins}-${s.loss}-${s.ties}`}</span>
                <span className="hp-num career-season-avg">
                  {signed(s.avg_margin)}
                </span>
